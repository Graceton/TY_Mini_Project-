...
class AccessibilityApp(QWidget):
    def __init__(self):
        ...
        self.ui_timer.timeout.connect(self.apply_ui_settings)
        self.ui_timer.start(500) # Check every 500ms
        self.apply_ui_settings() # Initial check
    ...
    def apply_ui_settings(self):
        """Apply High Contrast and Large UI settings dynamically."""
        is_high_contrast = self.settings_manager.get("high_contrast")
        is_large_ui = self.settings_manager.get("large_ui")
        
        # Scaling factors
        font_size = 32 if is_large_ui else 22
        btn_height = 90 if is_large_ui else 60
        sub_btn_height = 80 if is_large_ui else 50
        
        # Theme colors
        contrast_style = ""
        if is_high_contrast:
            contrast_style = "background-color: black; color: yellow; border: 2px solid yellow;"
        ...
        for btn in self.all_buttons:
            # We need to preserve the button's original background color if not in high contrast
            # Extracting color from the button's current style if possible, or just re-coloring
            # A cleaner way is to store the base color
            base_color = btn.property("base_color")
            if not base_color: continue
            ...
            if btn.height() != btn_height and btn.height() > 55: # Main buttons
                 btn.setFixedHeight(btn_height)
            elif btn.height() != sub_btn_height and btn.height() <= 55: # Sub buttons
                 btn.setFixedHeight(sub_btn_height)
