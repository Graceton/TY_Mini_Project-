...
class AccessibilityApp(QWidget):
    ...
    def initUI(self):
        ...
        # Main buttons
        self.zoom_btn = self.createButton("≡ƒöì", "Zoom Options", "#3498db")
        self.reader_btn = self.createButton("≡ƒöè", "Reader Options (Tab+R)", "#2ecc71")
        self.voice_btn = self.createButton("≡ƒÄÖ∩╕Å", "Voice Assistant", "#e74c3c")
        self.settings_btn = self.createButton("ΓÜÖ∩╕Å", "Settings", "#7c37cd", 50)
        self.exit_btn = self.createButton("Γ¥î", "Exit / Minimize", "#2c3e50", 50)
    ...
    def expandZoomButtons(self):
        ...
        reset_btn = self.createButton("≡ƒöä", "Reset Zoom", "#3498db", 50)
    ...
    def createZoomOptions(self):
        layout = QHBoxLayout()
        upper = self.createButton("≡ƒ¬ƒ", " Magnifier Window", "#3498db", 50)
        full = self.createButton("Γ¢╢", "Full Window Magnifier", "#3498db", 50)
        hover = self.createButton("≡ƒû▒∩╕Å", "Hover Magnifier", "#3498db", 50)
    ...
    def createReaderOptions(self):
        layout = QHBoxLayout()
        hover = self.createButton("≡ƒû▒∩╕Å", "Hover to Read", "#27ae60", 50)
        para = self.createButton("∩┐╜", "Paragraph Selection", "#27ae60", 50)
        line = self.createButton("≡ƒôÅ", "Line-wise Reader", "#27ae60", 50)
        ocr = self.createButton("≡ƒô╖", "OCR Overlay", "#27ae60", 50)
    ...
    def createVoiceOptions(self):
        layout = QHBoxLayout()
        start = self.createButton("≡ƒÄÖ∩╕Å", "Start Voice Assistant", "#c0392b", 50)
        stop = self.createButton("≡ƒöç", "Stop Voice Assistant", "#c0392b", 50)
    ...
    def createExitOptions(self):
        layout = QHBoxLayout()
        minimize = self.createButton("Γ₧û", "Minimize App", "#34495e", 50)
        quit_app = self.createButton("Γ¥î", "Quit App", "#34495e", 50)
    ...
    def keyPressEvent(self, event):
        if event.modifiers() == Qt.ControlModifier:
            if event.key() in (Qt.Key_Plus, Qt.Key_Equal):
                self.send_zoom_command("zoom_in")
            elif event.key() == Qt.Key_Minus:
                self.send_zoom_command("zoom_out")
